...
if TYPE_CHECKING:
    from piano_app.domain.score.document.models.structural.temporal_anchor import (
        TemporalAnchor,
    )

_DEFAULT_TIME_SIGNATURE: RhythmicSize = RhythmicSize(
    value=DottedRhythmicValue(
        value=RhythmicValue.QUARTER,
    ),
    count=4,
)
...
class Measure(ScoreEntity):
    ...
    def create(
        cls,
        *,
        time_signature: RhythmicSize = _DEFAULT_TIME_SIGNATURE,
    ) -> Measure:
