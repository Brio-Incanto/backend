...
class ScoreEntityResolver:
    """Resolves raw entity ids (e.g. from the frontend) to the live entities of
    a score."""
    ...
    def resolve(self, *, ids: set[str]) -> dict[str, ScoreEntity]:
        """Return the requested entities keyed by id. Missing ids are simply
        absent from the result; the caller decides how to treat them."""
