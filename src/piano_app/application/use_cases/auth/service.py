...
from piano_app.application.errors import (
    IdentityAlreadyLinkedError as AppIdentityAlreadyLinkedError,
)
from piano_app.application.errors import InvalidAccessTokenError as AppInvalidAccessTokenError
from piano_app.application.errors import (
    InvalidExternalCredentialError as AppInvalidExternalCredentialError,
)
from piano_app.application.errors import InvalidRefreshTokenError as AppInvalidRefreshTokenError
from piano_app.application.errors import UsernameConflictError as AppUsernameConflictError
from piano_app.application.errors import UsernameRequiredError
...
class AuthService:
    ...
    async def sign_in(
        ...
        *,
        ...
        try:
            ...
            raise AppInvalidExternalCredentialError from None
        ...
        async with self._auth_uow_factory() as uow:
            ...
            if user_id is None:
                ...
                #   - IdentityAlreadyLinkedError: the winner linked OUR identity, so
                #     the re-read always finds it -> we sign in as that user.
                #   - UsernameConflictError: if the re-read finds our identity it was
                ...
                except (IdentityAlreadyLinkedError, UsernameConflictError) as error:
                    await uow.rollback()
                    user_id = await uow.identity_repository.find_user_id(identity=identity)
                    if user_id is None:
                        if isinstance(error, UsernameConflictError):
                            raise AppUsernameConflictError(username=error.username) from error
                        raise AppIdentityAlreadyLinkedError from error
    ...
    async def refresh(self, *, credential: str) -> AuthSession:
        async with self._auth_uow_factory() as uow:
            ...
            if rotated is None:
                raise AppInvalidRefreshTokenError
    ...
    async def get_current_user(self, *, user_id: str) -> UserProfile:
        async with self._auth_uow_factory() as uow:
            ...
            if user is None:
                raise AppInvalidAccessTokenError
